...
const props = defineProps({ record: {type: Object, required: true}, year: {type: Number, required: true} });

function age(person) {
  let age = person?.Age || person?.age;
  if (person) {
    if (!person) {
      return 'N/A';
    }
    if (!person?.age) {
      return props.year - person.birth_year;
    }
    if (age > 0) {
      return `${ age } years`;
    }
    ...
    if (age > 0 && (person?.age_months === 0 || person?.age_months === '')) {
      return `${ age } years`;
    }
    if (age > 0 && person?.age_months > 0) {
      return `${ age } years, ${ person?.age_months } months`;
    }
    if (age > 0 && person?.age_months < 0) {
      return `${ age } years`;
    }
...
<template>
  <p><strong>Age:</strong> {{ record?.age || year - record?.birth_year || 'N/A' }}</p>
